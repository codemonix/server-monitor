...
export default function SystemSettings() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setStatus({
                type: "success",
                text: "Global configuration updated. Agents will pick this up on their next token refresh.",
            });
        } catch (error) {
            setStatus({
                type: "error",
                text: error.response?.data?.error || "Failed to update global configuration.",
            });
        } finally {
            setLoading(false);
        }   
    ...
    return (
        <Paper sx={{ p: 3 }} >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, maxWidth: 400 }} >
                ...
                {status.text && (
                    <Alert severity={status.type as "success" | "error" | "info" | "warning"}>{status.text}</Alert>
                )}
