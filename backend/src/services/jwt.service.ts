...
export function signAccess(user) {
    return jwt.sign({ sub: user._id, role: user.role, typ: "user" }, ACCESS_SECRET, accessSignOpts());
}

export function signRefresh(user) {
    return jwt.sign({ sub: user._id, role: user.role, typ: "refresh" }, REFRESH_SECRET, refreshSignOpts());
}
...
export function signAgent(agent) {
    return jwt.sign({ sub: agent._id, typ: "agent" }, AGENT_SECRET, { expiresIn: "7d" } satisfies SignOptions);
}
...
export const agentJwtService = {
    async issueToken(agent) {
        ...
        const accessToken = jwt.sign(payload, AGENT_SECRET, { expiresIn: AGENT_ACCESS_TTL } as SignOptions);
        const refreshToken = jwt.sign(payload, AGENT_SECRET, { expiresIn: AGENT_REFRESH_TTL } as SignOptions);
    ...
    async refreshTokens(refreshToken) {
        try {
            ...
            const payload = { sub: agent._id, typ: 'agent' };
            const newAccess = jwt.sign(payload, AGENT_SECRET, { expiresIn: AGENT_ACCESS_TTL } as SignOptions);
            const newRefresh = jwt.sign(payload, AGENT_SECRET, { expiresIn: AGENT_REFRESH_TTL } as SignOptions);
